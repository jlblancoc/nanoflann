...
proc = subprocess.Popen(["../../build/bin/./nanoflann_testRandom"], stdout=subprocess.PIPE, shell=True)
...
proc = subprocess.Popen(["../../build/bin/./flann_testRandom"], stdout=subprocess.PIPE, shell=True)
(out, err) = proc.communicate()
flannList = out.split()
...
'''
plt.axis([0, 10.5, -0.000001, 0.000018])
plt.xlabel('Relative size of point cloud', fontsize=25)
plt.ylabel('Time (sec)', fontsize=25)
